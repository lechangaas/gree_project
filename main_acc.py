...
if __name__ == '__main__':
    def build_argparser():
        ...
        parser.add_argument('-gl', '--generate_label', help='generate label.txt', action='store_true')

        return parser


    args = build_argparser().parse_args()
    gl_flag = args.generate_label
    if gl_flag:
        label_file = 'label.txt'
        f_label_file = open(label_file, 'w')
    ...
    if label_file is not None:
        ...
        good_full_folder = os.path.join(output_folder, 'good_full')
        good_no_folder = os.path.join(output_folder, 'good_no')
        error_less_folder = os.path.join(output_folder, 'error_less')
        error_more_folder = os.path.join(output_folder, 'error_more')
        if not os.path.exists(good_full_folder):
            os.makedirs(os.path.join(good_full_folder))
        if not os.path.exists(good_no_folder):
            os.makedirs(os.path.join(good_no_folder))
        if not os.path.exists(error_less_folder):
            os.makedirs(os.path.join(error_less_folder))
        if not os.path.exists(error_more_folder):
            os.makedirs(os.path.join(error_more_folder))
    else:
        acc_flag = False
    ...
    for name in tqdm(sorted_image_names):
        # if label_dict[name] > 4:
        if 1:
            ...
            if ret == 0:
                ...
                for ratio in crop_ratio:
                    ...
                    hole_circles = detector.find_holes(detect_hole_image.copy(), cfg)
                    # visualize
                    ...
                    if hole_circles is not None:
                        ...
                        pred_hole_num = len(hole_circles)
                    else:
                        pred_hole_num = 0
            else:
                pred_hole_num = 0
            ...
            # put text on image
            cv2.putText(image, name, (5, 25), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), thickness=1)
            cv2.putText(image, 'pred holes:%d' % pred_hole_num, (5, 50), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255),
                        thickness=1)
            if acc_flag:
                gt_hole_num = label_dict[name]
                cv2.putText(image, 'gt holes:%d' % gt_hole_num, (5, 75), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255),
                            thickness=1)
            ...
            if not acc_flag:
                ...
                gt_hole_num = label_dict[name]
                if gt_hole_num == pred_hole_num:
                    if gt_hole_num == 0:
                        cv2.imwrite(os.path.join(good_no_folder, name), image)
                    else:
                        cv2.imwrite(os.path.join(good_full_folder, name), image)
                elif gt_hole_num < pred_hole_num:
                    cv2.imwrite(os.path.join(error_more_folder, name), image)
                else:
                    cv2.imwrite(os.path.join(error_less_folder, name), image)

            if gl_flag:
                real_hole_circles = len(hole_circles) if hole_circles is not None else 0
                write_str = "%s %d\n" % (os.path.basename(image_name), real_hole_circles)
                f_label_file.write(write_str)
    ...
    print("ratio_min:%f\n"
          ...
          "outter_min:%f\n"
          "outter_max:%f"
          % (detector.min_ratio, detector.max_ratio, detector.min_inner, detector.max_inner,
             detector.min_contrast, detector.max_contrast, detector.min_outter, detector.max_outter))
